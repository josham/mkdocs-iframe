...
from duty.callables import black, blacken_docs, coverage, lazy, mkdocs, mypy, pytest, ruff
...
def check_docs(ctx: Context) -> None:
    ...
    Parameters:
        ctx: The context instance (passed automatically).
    """
    Path("htmltest").mkdir(parents=True, exist_ok=True)
    Path("htmltest/index.html").touch(exist_ok=True)
...
def release(ctx: Context, version: str) -> None:
    ...
    if not TESTING:
        ...
        ctx.run("pdm publish", title="Building/Publishing dist/wheel", pty=PTY)
        docs_deploy.run()
